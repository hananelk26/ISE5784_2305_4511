...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        final var result1 = sphere.findIntersections(new Ray(p_100, v310))
                .stream().sorted(Comparator.comparingDouble(p -> p.distance(p_100))).toList();
        assertEquals(2, result1.size(), "Wrong number of points");
        assertEquals(exp, result1, "Ray crosses sphere");

        // TC03: Ray starts inside the sphere (1 point)
        assertEquals(new Point(0, sqrt(0.75), 1), sphere.findIntersections
                        (new Ray(new Point(0, 0, 0.5), new Vector(0,1,0))),
                "Fail ray starts inside the sphere (1 point)");

        // TC04: Ray starts after the sphere (0 points)
        assertNull(sphere.findIntersections(new Ray(p100, v110)),
                "Fail ray starts after the sphere (0 points)");
        ...
        assertEquals(p0_11, sphere.findIntersections(new Ray(p101,new Vector(1,1,0))),
                "Fail ray starts at sphere and goes inside (1 point)");

        // TC12: Ray starts at sphere and goes outside (0 points)
        assertNull(sphere.findIntersections(new Ray(p101,new Vector(-1,-1,0))),
                "Fail ray starts at sphere and goes outside (0 points)");
        ...
        assertEquals(List.of(p000, p002), result2,
                "Fail ray starts before the sphere (2 points)");

        // TC14: Ray starts at sphere and goes inside (1 point)
        assertEquals(p002,sphere.findIntersections(new Ray(p000,v001)),
                "Fail ray starts at sphere and goes inside (1 point)");

        // TC15: Ray starts inside (1 point)
        assertEquals(p002,sphere.findIntersections(new Ray(new Point(0, 0, 0.5),v001)),
                "Fail Ray starts inside (1 point)");

        // TC16: Ray starts at the center (1 point)
        assertEquals(p002,sphere.findIntersections(new Ray(p001,v001)),
                "Fail ray starts at the center (1 point)");
        ...
        assertNull(sphere.findIntersections(new Ray(p_100,v100)),
                "Fail ray starts before the tangent point (0 points)");

        // TC20: Ray starts at the tangent point
        assertNull(sphere.findIntersections(new Ray(p002,v100)),
                "Fail ray starts at the tangent point (0 points)");

        // TC21: Ray starts after the tangent point
        assertNull(sphere.findIntersections(new Ray(p_100,new Vector(-1,0,0))),
                "Fail ray starts after the tangent point (0 points)");
