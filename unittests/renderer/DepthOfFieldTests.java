...
public class DepthOfFieldTests {
    ...
    public void depthOfFieldImage() {
        ...
        scene.lights.add(new PointLight(new Point(-50, -50, -100), new Color(WHITE)).setKl(0.0001).setKq(0.0001));

        camera.setRayTracer(new SimpleRayTracer(scene)).setImageWriter(new ImageWriter("depthOfField", 800, 400))
                .build().renderImageDOF().writeToImage();
    }
}
